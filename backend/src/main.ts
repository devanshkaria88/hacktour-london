...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .addCookieAuth(
      process.env.SESSION_COOKIE_NAME ?? 'olando_session',
      { type: 'apiKey', in: 'cookie', name: process.env.SESSION_COOKIE_NAME ?? 'olando_session' },
      'session-cookie',
    )
    .addApiKey(
  ...
  Logger.log(`Olando API listening on http://localhost:${port}`, 'Bootstrap');
